...
def break_korean(string):
    ...
    for k in word_list:
        if(ord(k) >= ord("가") and ord(k) <= ord("힣")):
            #유니코드상 몇번째 글자인지 인덱스 구하기
            char_index = ord(k) - ord("가")

            chosung = int((char_index / 28) / 21)
            break_word.append(CHO[chosung])

            jungsung = int((char_index / 28) % 21)
            break_word.append(JUNG[jungsung])

            jongsung = int(char_index % 28)

            if jongsung > 0:
                break_word.append(JONG[jongsung])
